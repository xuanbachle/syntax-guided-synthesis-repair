...
rootdir=`pwd` #"/home/ubuntu/codeforces-crawler/CodeforcesSpider" #directory of this script
versiondir="/emseoverfitting/codeflaws" #"$rootdir/solutions-short"  # File contains all subjects (buggy code, golden code, and test cases)
filename1=$1 #"/emseoverfitting/script/codeflaws/all-script/allbugs.txt"  # File contains list of subjects in versiondir
currentTime=$(date +"%Y-%m-%d-%H-%M-%S")
rundir="$rootdir/angelix-run-$currentTime"  # Directory for running the test script
instrumentprintf="/angelix/build/llvm-3.7.0.src/build/bin/instrument-printf"
angelixexperimentsdir=$2 #"/emseoverfitting/angelix-experiments" # directory in which angelix is called from
...
if [ $# -lt 1 ]; then
  ...
  if [ "$3" == "--use-nsynth" ]; then
    runNewSynthesizer="--use-nsynth"
    echo -e "$3 option enabled.\n"
  fi  

  if [ "$3" == "--semfix" ]; then
    runNewSynthesizer="--semfix"
    echo -e "$3 option enabled.\n"
  fi
  
  if [ "$3" == "--synthesis-other-solver Enum" ]; then
    runNewSynthesizer="$3"
    echo -e "$3 option enabled.\n"
  fi


  if [ "$3" == "--synthesis-other-solver CVC4" ]; then
    runNewSynthesizer="$3"
    echo -e "$3 option enabled.\n"
  fi

  if [ "$4" == "--all-tests" ]; then
    runNewSynthesizer+=" --all-tests"
    echo -e "$3 option enabled.\n"
  fi

fi

echo "params: "$runNewSynthesizer
...
#    done
percent=1.0  
numpos=${#positiveCases[@]}
numneg=${#negativeCases[@]}
total=$(($numpos + $numneg))
echo "npos"$numpos
echo "nneg"$numneg

pratio=`echo "$numpos / ($numneg+$numpos)" | bc -l`

echo "ratio:"$pratio
rtotal=`echo "$percent*$total" | bc -l`
rtotalround=`echo $rtotal | awk '{print int($1+0.5)}'`
echo "rtotal:"$rtotal
rpos=`echo "$rtotal*$pratio" | bc -l`
rposround=`echo $rpos | awk '{print int($1-0.5)}'`
echo "rposround:"$rposround
rneg=$(($rtotalround-$rposround))
rneground=`echo $rneg | awk '{print int($1-0.0)}'`
echo "rneground:"$rneground

shufPos=( $(shuf -e "${positiveCases[@]}") )
shufNeg=( $(shuf -e "${negativeCases[@]}") )

pindex=0
for p in "${shufPos[@]}"
do
       if [[ "$pindex" -ge "$sposround" ]]; then
         break
       fi
       pindex=$(($pindex+1))
       buggyTestCases+="$p "
