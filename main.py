...
#"/home/dxble/workspace/angelix-experiments/coreutilsexp/coreutils/" "src/sort.c" "/home/dxble/workspace/angelix-experiments/coreutilsexp//coreutils-sort-oracle" 1 2 3 --assert "/home/dxble/workspace/angelix-experiments/coreutilsexp/assert-sort.json" --verbose --configure "./configure CFLAGS=-std=c99" --build "/tmp/build-sort"
...
class Angelix:
    ...
    def generate_patch(self):
        ...
        print ("suspicious = ",)
        print (suspicious)
        #suspicious = [[(2282,11, 2282,36), (2282,11, 2282,36)]]
        #tests=positive
        #tests+=negative
        ...
        #cpsuspicious=deepcopy(suspicious)
        #solutions=0
        #while True:
            if self.config['use_semfix_syn']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--synthesis-timeout', metavar='MS', type=int, default=90, # 30 sec
                        help='synthesis timeout (default: %(default)s)')
    parser.add_argument('--synthesis-other-solver', metavar='SOLVER', type=str, default=None,  # 30 sec
                        help='User other synthesis solver (default: %(default)s)')
    parser.add_argument('--synthesis-simplify', action='store_true', default=True,
                            help='Ignore simple or trivial solution, e.g., constant (default: %(default)s)')
    parser.add_argument('--space-reduction', action='store_true', default=True,
                        help='Reduce search space, e.g., use only vars from old expr (default: %(default)s)')
    ...
    #myargs=['src/', 'digits.c', 'oracle_bb 1 3 2 5 4 6', '--assert' , 'assert_bb.json' , '--test-timeout',
    #        '5000', '--timeout' , '3600',  '--verbose' , '--klee-timeout', '10', '--klee-solver-timeout', '10']
    ...
    config['synthesis_other_solver'] = args.synthesis_other_solver
    config['synthesis_simplify'] = args.synthesis_simplify
    config['space_reduction'] = args.space_reduction
